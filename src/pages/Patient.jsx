...
import { patients } from '../constant/patients';
const Patient = () => {
  useEffect(() => {
  ...
  {patients.map((patient, index) => (
    ...
    >
      <div className="md:w-1/2 text-center md:text-left" data-aos="fade-up">
        <h2 className="text-3xl font-semibold mb-2">{patient.name}</h2>
        <p className="text-lg text-gray-200 mb-4">Age: {patient.age}</p>
        <p className="text-base text-gray-300">{patient.description}</p>
      </div>
      <div className="md:w-1/2 mt-6 md:mt-0 flex justify-center" data-aos="zoom-in">
        <img src={patient.image} alt={patient.name} className="w-full max-w-xs h-auto rounded-lg shadow-2xl" />
      </div>
    </div>
  ))}
</div>
        ...
        <div className="mt-12 flex flex-col md:flex-row items-center space-y-6 md:space-y-0 md:space-x-6 p-4" data-aos="fade-up">
          ...
          <div className="md:w-1/2" data-aos="zoom-in">
            <img src="/clinik.webp" alt="Patient Care" className="w-full h-auto rounded-lg shadow-2xl" />
          </div>
        </div>
      </div>
    </div>
